...
class cmb_Meta_Box {
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear', 'restyle' ),
					'defaultString' => __( 'Default', 'restyle' ),
					'pick'          => __( 'Select Color', 'restyle' ),
					'current'       => __( 'Current Color', 'restyle' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'upload_file'     => __( 'Use this file', 'restyle' ),
			'remove_image'    => __( 'Remove Image', 'restyle' ),
			'remove_file'     => __( 'Remove', 'restyle' ),
			'file'            => __( 'File:', 'restyle' ),
			'download'        => __( 'Download', 'restyle' ),
			'ajaxurl'         => admin_url( '/admin-ajax.php' ),
			'up_arrow'        => '[#8593]&nbsp;',
			'down_arrow'      => '&nbsp;[#8595]',
			'check_toggle'    => __( 'Select / Deselect All', 'restyle' ),
		) ) );
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'restyle' ) );
